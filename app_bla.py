...
# params = list(votes_df)
# max_length = len(votes_df)
...
#
#
def load_data_value_store():
    # Initialize store data
    df_referendum = get_df_referendum()
    return df_referendum.to_dict()
...
def build_tab_1():
    return [
        ...
        html.Div(
            ...
            children=[html.Br(), dcc.RangeSlider(65, 217),],
        ),
        #
        html.Div(
            id="settings-menu",
...
app.layout = html.Div(
    ...
    children=[
        ...
        dcc.Store(id="value-data", data=load_data_value_store(), storage_type='memory'),
        # dcc.Store(
        #     id='value-data',
        #     data=init_data_value_store()
        # ),
...
#         data[param]['ooc'] = populate_ooc(votes_df[param], ucl, lcl)
#         return data
#
#
# @app.callback(
...
#     ],
#     state=[State("value-data", "data"), State("control-chart-live", "figure")],
# )
...
def update_bar_chart(df_votes):
    ...
    df_delegated_count_sum = (
        df_delegated.groupby("id")["accountId"]
            .count()
            .reset_index(name="vote_counts")
            .sort_values(by="id")
    )
...
def update_pie_chart(n_intervals, stored_data):
    ...
    print(df["call_module"].value_counts().index.tolist())
    return new_figure
